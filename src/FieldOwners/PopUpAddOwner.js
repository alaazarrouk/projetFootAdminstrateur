...
const PopUpAddOwner = ({ refresh, onChange }) => {
  const states = [
    "Not-Selected",
    "Ariana",
    "Ben arous",
  ...
  const handleSubmit = async (event) => {
    ...
    if (val) {
      await axios.post(`/register`, person).then((response) => {
        toast.success("Field Owner successfully created", {
  ...
  return (
    <div className="PopUpAddOwner">
      <ToastContainer />
      <h2> Create Field Owner</h2>
